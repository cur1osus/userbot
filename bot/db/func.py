...
class RedisStorage:
    ...
    async def save(self, key: Any, value: Any, ttl: int | None = None, **kwargs) -> None:
        """Alias for set with optional TTL to match RedisClient interface."""
        options = kwargs.copy()
        if ttl is not None:
            options.setdefault("ex", ttl)
